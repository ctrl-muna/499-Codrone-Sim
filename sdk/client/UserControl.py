...
class UserControl:
    def __init__(self):
        ...
        self.scene_config_name = "scene_basic_drone.jsonc" ###I defined the scene config name here so it can be used in both connect() and resetSimulator() without hardcoding it in both places.
    ...
    def connect(self):
        try:
            ...
            self.client.connect()
            try:
                self.world = World(self.client, self.scene_config_name, delay_after_load_sec=2)###i replaced "scene_basic_drone.jsonc" with self.scene_config_name so that "scene_basic_drone.jsonc" is only defined in one place and doesn't need to be hardcoded in multiple places.
            except Exception as e:
                projectairsim_log().error(f"Error creating world: {e}")

            self.drone = Drone(self.client, self.world, "Drone1")
            self._subscribe_drone_topics()  # <-- replaces the two client.subscribe calls that were previously in connect()
    ...
    def _subscribe_drone_topics(self): ###new function to subscribe to drone topics, called from connect() and resetSimulator() after initializing the drone object.
    ###Subscribe to pose and collision topics for the current drone object. These are the two client.subscribe calls that were previously in connect(), now moved here so they can be reused in resetSimulator() after reloading the scene.
    ...
    def save_Command(self, command, Duration, status="ok", notes=""):
        ...
        Command_message = f"{current_Date},{command},{Duration},{status},{notes}" ## added the status and notes fields to the command message for acknowledgment of command success or failure and any relevant notes.
    ...
    def resetSimulator(self): ###I replaced the old resetDrone() with this function
    ...
    ###and returns everything to its initial pose, equivalent to a fresh launch.
        ...
        try:
            ...
            projectairsim_log().error(f"resetSimulator: failed to reload scene — {e}") ###replaced the old "Error during reset: {e}" with a more specific message about scene reload failure
            return
        ...
        try:
            ...
            projectairsim_log().error(f"resetSimulator: drone re-initialization failed — {e}") ###replaced the old "Error during reset: {e}" with a more specific message about drone re-initialization failure.
    ...
    async def commandParse(self, com, dur):
        ...
        if com == "Close":
            ...
            projectairsim_log().info("Reset: initiating simulator-level scene reload...")###replaced the old info message "Resetting Drone and Environment)"
            self.resetSimulator()###took self.restDrone() and self.takeOff = False.
        elif com == "Takeoff" or com == "Land":
            if com == "Takeoff":
                if self.takeOff:
                    projectairsim_log().info("Drone is has already taken off.")
                else:##took off the old command block and put a new try-except block around the takeoff_async command to catch and log any errors that occur during takeoff, and to save the command with a "failed" status if an error occurs. This way we can have a record of failed takeoff attempts in the Command CSV file along with the error message for debugging purposes (new lines 144-155).
                    try:
                        ...
                        self.save_Command(com, dur, status="ok", notes="")  # save on success
                    except Exception as e:
                ...
                else:##took off the old command block and put a new try-except block around the takeoff_async command to catch and log any errors that occur during takeoff, and to save the command with a "failed" status if an error occurs. This way we can have a record of failed takeoff attempts in the Command CSV file along with the error message for debugging purposes (new lines 159-171).
                    try:
            ...
            if not self.takeOff:
                projectairsim_log().info("Drone is not in the air.")
            else: ##put the code in a try-except block
                try:
                    speed = 5
                    Degrees = 90
                    ...
                    ###added 192-197 to log the invocation and completion of the yaw command, and to save the command with an "ok" status on success or a "failed" status with the error message on failure. This way we can have a record of yaw commands in the Command CSV file along with their success or failure status and any relevant notes for debugging purposes.
            ...
            if not self.takeOff:
                projectairsim_log().info("Drone is not in the air.")
            else:##put everything in a try-except block
                try:
                    ...
                    await self.statePoll(1)
                    ##added 251-256 to save the command with either "Ok" status or "Failed" status and the error message on failure, so we can have a record of movement commands in the Command CSV file along with their success or failure status and any relevant notes for debugging purposes.
                    self.save_Command(com, dur, status="ok", notes="")  # save on success
                except Exception as e:
                    projectairsim_log().error(f"{com}: FAILED — {e}")
                    self.save_Command(com, dur, status="failed", notes=str(e))
